...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyImport.preview.selectFile")} />
    );
  }
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("companyImport.conflict.renames")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("companyImport.conflict.itemCount", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("companyImport.conflict.skipped") : t("companyImport.conflict.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyImport.conflict.confirmed")}
                      </>
                    ) : (
                      t("companyImport.conflict.confirmRename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("companyImport.adapter.title")}</h3>
          <span className="text-xs text-muted-foreground">
            {t("companyImport.adapter.agentCount", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  >
                    ...
                    {t("companyImport.adapter.configureAdapter")}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(file: File, t: (key: string, opts?: Record<string, unknown>) => string): Promise<{
  name: string;
  rootPath: string | null;
  files: Record<string, CompanyPortabilityFileEntry>;
}> {
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("companyImport.error.selectZip"));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("companyImport.error.noFilesInZip"));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("companyImport.source.zipHelpText");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyImport.breadcrumb.orgChart"), href: "/org" },
      { label: t("companyImport.breadcrumb.import") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("companyImport.error.noSource"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.error.previewFailed"),
        body: err instanceof Error ? err.message : t("companyImport.error.failedToPreview"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyImport.error.importComplete"),
        body: t("companyImport.error.importResult", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.error.importFailed"),
        body: err instanceof Error ? err.message : t("companyImport.error.failedToApply"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("companyImport.error.packageReadFailed"),
        body: err instanceof Error ? err.message : t("companyImport.error.failedToReadFolder"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("companyImport.empty.selectCompany")} />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyImport.source.description")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("companyImport.source.githubRepo") },
              { key: "local", icon: Upload, label: t("companyImport.source.localZip") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("companyImport.source.chooseZip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("companyImport.source.zipFileCount", { name: localPackage.name, count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
            label={t("companyImport.source.githubUrl")}
            hint={t("companyImport.source.githubHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("companyImport.source.githubPlaceholder")}
              onChange={(e) => {
        ...
        <Field label={t("companyImport.source.target")} hint={t("companyImport.source.targetHint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("companyImport.source.createNewCompany")}</option>
            <option value="existing">
              {t("companyImport.source.existingCompany", { name: selectedCompany?.name })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("companyImport.source.newCompanyName")}
            hint={t("companyImport.source.newCompanyHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("companyImport.source.newCompanyPlaceholder")}
            />
          </Field>
        )}

        <Field
          label={t("companyImport.source.collisionStrategy")}
          hint={t("companyImport.source.collisionHint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("companyImport.source.renameOnConflict")}</option>
            <option value="skip">{t("companyImport.source.skipOnConflict")}</option>
            <option value="replace">{t("companyImport.source.replaceExisting")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("companyImport.source.previewing") : t("companyImport.source.previewImport")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("companyImport.header.importPreview")}
              </span>
              <span className="text-muted-foreground">
                {t("companyImport.header.fileCount", { selected: selectedCount, total: totalFiles, count: selectedCount })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyImport.header.conflictCount", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyImport.header.errorCount", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ? t("companyImport.header.importing")
                : t("companyImport.header.importButton", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("companyImport.sidebar.packageFiles")}</h2>
              </div>
