...
const concurrencyPolicyKeys: Record<string, string> = {
  coalesce_if_active: "routines.concurrencyPolicy.coalesce_if_active",
  always_enqueue: "routines.concurrencyPolicy.always_enqueue",
  skip_if_active: "routines.concurrencyPolicy.skip_if_active",
};
const catchUpPolicyKeys: Record<string, string> = {
  skip_missed: "routines.catchUpPolicy.skip_missed",
  enqueue_missed_with_cap: "routines.catchUpPolicy.enqueue_missed_with_cap",
};
...
export function buildRoutineGroups(
  ...
  const noProjectLabel = t ? t("routines.noProject") : "No project";
  const unknownProjectLabel = t ? t("routines.unknownProject") : "Unknown project";
  const unassignedLabel = t ? t("routines.unassigned") : "Unassigned";
  const unknownAgentLabel = t ? t("routines.unknownAgent") : "Unknown agent";
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: routine.assigneeAgentId
          ? t("routines.toast.createdWithAssignee")
          : t("routines.toast.createdDraft"),
        tone: "success",
      });
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.tabs.routines") },
            { value: "runs", label: t("routines.tabs.recentRuns") },
          ]}
        />
            ...
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.sortLabel")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.sortLabel")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routines.sort.updated")],
                      ["created", t("routines.sort.created")],
                      ["lastRun", t("routines.sort.lastRun")],
                      ["title", t("routines.sort.title")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routines.sort.asc") : t("routines.sort.desc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.groupLabel")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.groupLabel")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["project", t("routines.group.project")],
                      ["assignee", t("routines.group.assignee")],
                      ["none", t("routines.group.none")],
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("routines.advancedDeliveryDesc")}</p>
                  </div>
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.error.createFailed")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines.error.loadFailed")}
          </CardContent>
        </Card>
      ) : null}
