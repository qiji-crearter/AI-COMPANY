...
function getExecutionWorkspaceBaseTabItems(t: (key: string) => string): OrderedExecutionWorkspaceTabItem[] {
  return [
    { value: "issues", label: t("executionWorkspace.tab.issues"), order: 10 },
    { value: "services", label: t("executionWorkspace.tab.services"), order: 20 },
    { value: "configuration", label: t("executionWorkspace.tab.configuration"), order: 30 },
    { value: "runtime_logs", label: t("executionWorkspace.tab.runtimeLogs"), order: 40 },
    { value: "routines", label: t("executionWorkspace.tab.routines"), order: 60 },
  ];
}
...
function parseWorkspaceRuntimeJson(value: string, t: (key: string) => string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("executionWorkspace.error.invalidJsonObject"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("executionWorkspace.error.invalidJson"),
    };
  }
}
...
function validateForm(form: WorkspaceFormState, t: (key: string) => string) {
  ...
  if (repoUrl) {
    try {
      ...
      return t("executionWorkspace.error.invalidRepoUrl");
    }
  }
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span>{t("executionWorkspace.lastRun", { date: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt, t) })}</span>
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("executionWorkspace.toast.routineStarted"),
        body: t("executionWorkspace.toast.routineStartedBody"),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("executionWorkspace.toast.routineRunFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("executionWorkspace.toast.routineRunFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("executionWorkspace.card.workspaceRoutines")}</CardTitle>
          <CardDescription>
            {t("executionWorkspace.card.workspaceRoutinesDescription")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("executionWorkspace.loading.routines")}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("executionWorkspace.error.failedToLoadRoutines")}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("executionWorkspace.empty.noRoutines")}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([...getExecutionWorkspaceBaseTabItems(t), ...workspacePluginTabItems]),
    [t, workspacePluginTabItems],
  );
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("executionWorkspace.breadcrumb.projects"), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("executionWorkspace.breadcrumb.workspaces"), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("executionWorkspace.error.failedToSave"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("executionWorkspace.toast.workspaceJobCompleted")
          : request.action === "stop"
            ? t("executionWorkspace.toast.workspaceServiceStopped")
            : request.action === "restart"
              ? t("executionWorkspace.toast.workspaceServiceRestarted")
              : t("executionWorkspace.toast.workspaceServiceStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("executionWorkspace.error.failedToControl"));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("executionWorkspace.loading.workspace")}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("executionWorkspace.error.failedToLoad")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("executionWorkspace.error.failedToBuildPatch"));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("executionWorkspace.serviceEmptyMessage.withConfig")
                : t("executionWorkspace.serviceEmptyMessage.withoutConfig")
            }
            jobEmptyMessage={t("executionWorkspace.jobEmptyMessage")}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("executionWorkspace.disabledHint")
            }
            onAction={(request) => controlRuntimeServices.mutate(request)}
          />
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.card.workspaceSettings")}</CardTitle>
                <CardDescription>
                  {t("executionWorkspace.card.workspaceSettingsDescription")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.section.general")}</div>
                  <Field label={t("executionWorkspace.field.workspaceName")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.workspaceNamePlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.section.sourceControl")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("executionWorkspace.field.branchName")} hint={t("executionWorkspace.field.branchNameHint")}>
                      <Input
                        className="font-mono"
                        value={form.branchName}
                        onChange={(event) => setForm((current) => current ? { ...current, branchName: event.target.value } : current)}
                        placeholder={t("executionWorkspace.field.branchNamePlaceholder")}
                      />
                    </Field>

                    <Field label={t("executionWorkspace.field.baseRef")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                        onChange={(event) => setForm((current) => current ? { ...current, baseRef: event.target.value } : current)}
                        placeholder={t("executionWorkspace.field.baseRefPlaceholder")}
                      />
                    </Field>
                  </div>

                  <Field label={t("executionWorkspace.field.repoUrl")}>
                    <Input
                      value={form.repoUrl}
                      onChange={(event) => setForm((current) => current ? { ...current, repoUrl: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.repoUrlPlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.section.paths")}</div>
                  <Field label={t("executionWorkspace.field.workingDirectory")}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                      onChange={(event) => setForm((current) => current ? { ...current, cwd: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.workingDirectoryPlaceholder")}
                    />
                  </Field>

                  <Field label={t("executionWorkspace.field.providerPathRef")}>
                    <Input
                      className="font-mono"
                      value={form.providerRef}
                      onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.providerPathRefPlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.section.lifecycleCommands")}</div>
                  <Field label={t("executionWorkspace.field.provisionCommand")} hint={t("executionWorkspace.field.provisionCommandHint")}>
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.provisionCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, provisionCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.provisionCommandPlaceholder")}
                    />
                  </Field>

                  <Field label={t("executionWorkspace.field.teardownCommand")} hint={t("executionWorkspace.field.teardownCommandHint")}>
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.teardownCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, teardownCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.teardownCommandPlaceholder")}
                    />
                  </Field>

                  <Field label={t("executionWorkspace.field.cleanupCommand")} hint={t("executionWorkspace.field.cleanupCommandHint")}>
                    <Textarea
                      className="min-h-16 font-mono"
                      value={form.cleanupCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspace.field.cleanupCommandPlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.section.runtimeConfig")}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("executionWorkspace.runtimeConfig.source")}
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("executionWorkspace.runtimeConfig.overriding")
                            : runtimeConfigSource === "project_workspace"
                              ? t("executionWorkspace.runtimeConfig.inheriting")
                              : t("executionWorkspace.runtimeConfig.none")}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("executionWorkspace.runtimeConfig.advancedJson")}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("executionWorkspace.runtimeConfig.advancedJsonDescription")}
                    </p>
                    <div className="mt-3">
                      <Field label={t("executionWorkspace.field.workspaceCommandsJson")} hint={t("executionWorkspace.field.workspaceCommandsJsonHint")}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("executionWorkspace.runtimeConfig.inheritCheckbox")}</label>
                        </div>
                        <Textarea
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.card.workspaceContext")}</CardTitle>
                <CardDescription>{t("executionWorkspace.card.workspaceContextDescription")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspace.detailRow.project")}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.projectWorkspace")}>
              ...
              <DetailRow label={t("executionWorkspace.detailRow.sourceIssue")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("executionWorkspace.detailRow.derivedFrom")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("executionWorkspace.detailRow.workspaceId")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.card.concreteLocation")}</CardTitle>
                <CardDescription>{t("executionWorkspace.card.concreteLocationDescription")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspace.detailRow.workingDir")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("executionWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.providerRef")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("executionWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.repoUrl")}>
              ...
              <DetailRow label={t("executionWorkspace.detailRow.baseRef")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("executionWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.branch")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("executionWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.opened")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.lastUsed")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspace.detailRow.cleanup")}>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("executionWorkspace.card.runtimeAndCleanupLogs")}</CardTitle>
              <CardDescription>{t("executionWorkspace.card.runtimeAndCleanupLogsDescription")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("executionWorkspace.loading.workspaceOperations")}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("executionWorkspace.error.failedToLoadOperations")}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("executionWorkspace.empty.noOperations")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("executionWorkspace.loading.plugin")}</CardContent>
          </Card>
