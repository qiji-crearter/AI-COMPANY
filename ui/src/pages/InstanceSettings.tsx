...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.breadcrumbs.heartbeats") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.heartbeats.updateError"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          count === 1
            ? t("instanceSettings.heartbeats.confirmFailSingle", { detail })
            : t("instanceSettings.heartbeats.confirmFailMultiple", { count, enabled: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.heartbeats.disableAllError"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.heartbeats.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.heartbeats.loadError")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.heartbeats.pageTitle")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.heartbeats.pageDescription")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.heartbeats.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.heartbeats.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("instanceSettings.heartbeats.company", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = t("instanceSettings.heartbeats.agent", { count: enabledCount });
              if (!window.confirm(t("instanceSettings.heartbeats.confirmDisable", { count: enabledCount, noun }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("instanceSettings.heartbeats.disabling") : t("instanceSettings.heartbeats.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.heartbeats.noMatch")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.heartbeats.badgeOn") : t("instanceSettings.heartbeats.badgeOff")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instanceSettings.heartbeats.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instanceSettings.heartbeats.disableTimerHeartbeat") : t("instanceSettings.heartbeats.enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
