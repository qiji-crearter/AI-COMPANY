...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.breadcrumbs.access") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceSettings.access.toastCompanyAccessUpdated"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceSettings.access.toastInstanceRoleUpdated"), tone: "success" });
    },
  });
  ...
  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("instanceSettings.access.forbidden")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("instanceSettings.access.loadError");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.access.pageTitle")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("instanceSettings.access.pageDescription")}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("instanceSettings.access.searchLabel")}</span>
            <input
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            ...
            <div className="text-sm text-muted-foreground">{t("instanceSettings.access.loadingUser")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("instanceSettings.access.loadUserAccessError")}
            </div>
          ) : (
            <>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("instanceSettings.access.savingCompanyAccess") : t("instanceSettings.access.saveCompanyAccess")}
                  </Button>
                </div>
              </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("instanceSettings.access.unset")} &bull; {membership.status}
                        </div>
                      </div>
