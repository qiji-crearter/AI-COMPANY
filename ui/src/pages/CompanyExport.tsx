      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyExport.preview.binaryNotAvailable")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyExport.toast.exportFailed"),
        body: err instanceof Error ? err.message : t("companyExport.toast.failedToLoad"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyExport.toast.exportDownloaded"),
        body: t("companyExport.toast.exportedCount", { count: resultCheckedFiles.size, name: result.rootPath }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyExport.toast.exportFailed"),
        body: err instanceof Error ? err.message : t("companyExport.toast.failedToBuild"),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("companyExport.empty.selectCompany")} />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message={t("companyExport.empty.loading")} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {t("companyExport.header.title", { companyName: selectedCompany?.name ?? t("companyExport.header.titleFallback") })}
            </span>
            <span className="text-muted-foreground">
              {t("companyExport.header.fileCount", { selected: selectedCount, total: totalFiles, count: selectedCount })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyExport.header.warningCount", { count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("companyExport.header.building")
              : t("companyExport.header.exportButton", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
        <aside className="flex flex-col border-r border-border overflow-hidden">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">{t("companyExport.sidebar.packageFiles")}</h2>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("companyExport.sidebar.searchPlaceholder")}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("companyExport.sidebar.showMoreIssues", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
