...
const SCOPE_KEYS: Record<CompanySearchScope, string> = {
...
const SUBGROUP_KEYS: Record<SubGroupKey, string> = {
  issues: "search.subgroups.issues",
  comments: "search.subgroups.comments",
  documents: "search.subgroups.documents",
  agents: "search.subgroups.agents",
  projects: "search.subgroups.projects",
};
...
function describeScope(scope: CompanySearchScope, t: (key: string) => string) {
  if (scope === "all") return t("search.scopes.all");
  return t(SCOPE_KEYS[scope]);
}
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {t(SCOPE_KEYS[value as CompanySearchScope])}
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            aria-label={t("search.ariaLabelQuery")}
    ...
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.initial.identifierLookup")}</span> type{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code>{" "}
            {t("search.initial.identifierLookupHint")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.initial.quotedPhrases")}</span>{" "}
            {t("search.initial.quotedPhrasesHint")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.initial.cmdK")}</span>{" "}
            {t("search.initial.cmdKHint")}
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("search.error.serverError", { status }) : t("search.error.serverFail")} {t("search.error.description")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("search.error.openIssuesFilter")}
          </Button>
        </div>
      </div>
    );
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.empty.description", { scope: describeScope(scope, t).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
          ...
          <Button onClick={navigateIssuesFallback} size="sm" variant="ghost">
            {t("search.empty.openIssuesFilter")}
          </Button>
        </div>
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          ...
          <li>{t("search.empty.tipQuotedPhrases")}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {t("search.results.count", { count: totalResults })} · {t("search.results.sortedByRelevance")}
        </span>
      ...
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={t(SUBGROUP_KEYS[group.key])}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={t(SUBGROUP_KEYS[group.key])}
                trailing={
