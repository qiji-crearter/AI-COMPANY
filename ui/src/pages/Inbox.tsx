...
function runFailureMessage(run: HeartbeatRun, t?: (key: string) => string): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? (t ? t("inbox.runFailureDefault") : "Run exited with an error.");
}
...
export function formatJoinRequestInboxLabel(
  ...
  const getLabel = t ?? ((s: string) => s);
  if (joinRequest.requestType !== "human") {
    return joinRequest.agentName ? `${getLabel("inbox.agentJoinRequest")}: ${joinRequest.agentName}` : getLabel("inbox.agentJoinRequest");
  }
  ...
  return getLabel("inbox.humanJoinRequest");
}
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{linkedAgentName ? t("inbox.failedRunWith", { agentName: linkedAgentName }) : t("inbox.failedRun")}</>
              )}
            </span>
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox.requestedFrom", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
...
export function Inbox() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.approve"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.reject"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.approveJoin"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.rejectJoin"));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.archiveIssue"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.error.undoArchive"));
    },
    onSuccess: (_data, id) => {
                ...
                <>
                  ...
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>{t("inbox.markAllReadTitle")}</DialogTitle>
                        <DialogDescription>
                          {t("inbox.markAllReadDesc", { count: unreadIssueIds.length })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
      ...
      {tab === "all" && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="everything">{t("inbox.categoryFilter.all")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inbox.categoryFilter.issues_i_touched")}</SelectItem>
              <SelectItem value="join_requests">{t("inbox.categoryFilter.join_requests")}</SelectItem>
              <SelectItem value="approvals">{t("inbox.categoryFilter.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("inbox.categoryFilter.failed_runs")}</SelectItem>
          ...
          {showApprovalsCategory && (
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">{t("inbox.approvalStatusFilter.all")}</SelectItem>
                <SelectItem value="actionable">{t("inbox.approvalStatusFilter.actionable")}</SelectItem>
                <SelectItem value="resolved">{t("inbox.approvalStatusFilter.resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {t("inbox.subTask", { count: childCount })}
                        </span>
                      ) : undefined}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("inbox.searchSection.archived") : t("inbox.searchSection.other")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("inbox.newIssueInGroup", { label: group.label })}
                              aria-label={t("inbox.newIssueInGroup", { label: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("inbox.budgetAlertPrefix")}{" "}
                      <span className="font-medium">{dashboard!.costs.monthUtilizationPercent}%</span>{" "}
                      {t("inbox.budgetAlertSuffix")}
                    </span>
                  </Link>
                  <button
                    type="button"
