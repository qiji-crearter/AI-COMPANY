...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSidebar.profile") },
    ]);
  }, [setBreadcrumbs]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profileSettings.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.updateError"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.uploadError"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.removeError"));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profileSettings.loadError")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("profileSettings.board");
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadHintWithCompany", { companyName: selectedCompany.name })
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("profileSettings.pageTitle")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("profileSettings.pageDescription")}
        </p>
      </div>
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.uploadHint")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
