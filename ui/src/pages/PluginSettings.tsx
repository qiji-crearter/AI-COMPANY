...
export function PluginSettings() {
  ...
  const pluginSettingsT = (key: string) => t("pluginSettings." + key);
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("pluginManager.breadcrumbPlugins"), href: "/instance/settings/plugins" },
            ...
            <section className="space-y-4">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("pluginSettings.configureFromEnvironments")}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("pluginSettings.environmentSettingsDesc", { driver: driverLabel || t("pluginSettings.thisPlugin") })}
                  </p>
                  <div className="mt-3">
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  {dashboardData ? (
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("pluginSettings.crashesDetail", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("pluginSettings.runtimeDiagnosticsUnavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("pluginSettings.selectCompanyLocalFolders")}
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("pluginSettings.failedToLoadLocalFolders")}
        </div>
      ) : null}
      {isLoading ? (
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("pluginSettings.localFolderSaved")
          : t("pluginSettings.localFolderSavedNeedsAttention"),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pluginSettings.failedToSaveLocalFolder") });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("pluginSettings.localFolderPathRequired") });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pluginSettings.localFolderMustBeAbsolute") });
      return;
    }
    setMessage(null);
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={t("pluginSettings.localFolderPathPlaceholder")}
          />
...
function RequirementList({
  ...
  return (
    <div className="space-y-2 rounded-md border border-border/60 px-3 py-2">
      ...
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("pluginSettings.rootNotInspected")}</p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginSettings.failedToSaveConfig") });
    },
  });
