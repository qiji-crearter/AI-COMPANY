  ...
  return (
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs.tiles.debits")}
          value={formatCents(debitCents)}
          subtitle={t("costs.tiles.debitsSubtitle", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs.tiles.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costs.tiles.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.tiles.net")}
          value={formatCents(netCents)}
          subtitle={t("costs.tiles.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.tiles.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.tiles.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs.breadcrumb")}</h1>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costs.tiles.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costs.tiles.inferenceSpendSubtitle", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.tiles.budget")}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costs.tiles.budgetOpen")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.tiles.budgetIncidentSummary", { pausedAgents: budgetData?.pausedAgentCount ?? 0, pausedProjects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.tiles.budgetUtilization", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("costs.tiles.budgetNoCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costs.tiles.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costs.tiles.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costs.tiles.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs.tiles.financeEventsSubtitle", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.budgetLabel", { amount: formatCents(spendData.summary.budgetCents) })
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.budgetUtilizationPercent", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.tokenBreakdown", { inTokens: formatTokens(row.inputTokens + row.cachedInputTokens), outTokens: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.runCountApi", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("costs.runCountSubscription", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                ...
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs.tiles.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs.tiles.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs.tiles.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs.tiles.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs.tiles.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs.tiles.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs.tiles.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs.tiles.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">{t("costs.scopeBudgets", { scopeType })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.scopeCompanyDesc")
                            : scopeType === "agent"
                              ? t("costs.scopeAgentDesc")
                              : t("costs.scopeProjectDesc")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
